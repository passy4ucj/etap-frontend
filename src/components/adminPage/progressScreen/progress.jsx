...
import ProgressInputModal from "./tableOperation/progressAddModal";
import * as actionTypes from "../../../store/actions/actionTypes";
import * as tableProps from "./progressTableData";
...
function Progress(props) {
    ...
    React.useEffect(() => {
        const temp = async () => {
            // fetch progress data api call
            setSpinner(true);
            let res = await tableProps.fetchProgressData(props.pageSize, props.currentPage, props.orderBy);
            console.log("res", res.data.data);
            // setting progress list to tableData props
            if (res.status === 200) {
    ...
    return (
        <div className='col-12 col-md-10 mx-auto'>
            <h1 className='mt-5 px-lg-5'>Progress</h1>
            ...
            <Table
                className="px-lg-5"
                tableData={props.progressTableData}
            ...
            {
                modalState && <ProgressInputModal setModalState={setModalState} />
            }
            <ToastContainer
                position="top-right"
                autoClose={5000}
...
const mapStateToProps = (state) => {
    return {
        progressTableData: state.progressTableRedUnixFor.progressTableData,
        total: state.progressTableRedUnixFor.total,
        currentPage: state.progressTableRedUnixFor.currentPage,
        numberOfPages: state.progressTableRedUnixFor.numberOfPages,
        pageSize: state.progressTableRedUnixFor.pageSize,
        itemCountInCurrentPage: state.progressTableRedUnixFor.itemCountInCurrentPage,
        orderBy: state.progressTableRedUnixFor.orderBy,
    };
}
const mapDispatchToProps = (dispatch) => {
    return {
        // dispatching actions returned by action creators
        setTableData: (progressTableData) => dispatch({ type: actionTypes.SET_PROGRESS_TABLE_DATA, progressTableData: progressTableData }),
